...
import org.atty.stm.model.AdvogadoVerificacao; // <<< IMPORTANTE
import org.atty.stm.model.dto.CadastroDTO;
import org.atty.stm.model.dto.UsuarioDTO;
import org.atty.stm.model.Usuario;
import org.atty.stm.repository.AdvogadoVerificacaoRepository; // <<< IMPORTANTE
...
public class UsuarioService {
    ...
    @Inject
    AdvogadoVerificacaoRepository advogadoVerificacaoRepository; // <<< INJEÇÃO DO REPOSITÓRIO
    ...
    public Usuario criarUsuario(CadastroDTO dto) {
        ...
        novoUsuario.ativo = true; // Todo usuário é ativo por padrão ao criar

        usuarioRepository.persist(novoUsuario);

        // --- CORREÇÃO: CRIA O REGISTRO DE VERIFICAÇÃO PARA ADVOGADOS ---
        if ("ADVOGADO".equals(novoUsuario.perfil)) {
            AdvogadoVerificacao verificacao = new AdvogadoVerificacao();
            verificacao.advogado = novoUsuario;
            verificacao.oab = novoUsuario.oab;
            verificacao.status = "PENDENTE";
            verificacao.dataSolicitacao = LocalDateTime.now();

            advogadoVerificacaoRepository.persist(verificacao);
        }
        // ---------------------------------------------------------------
    ...
    public UsuarioDTO aprovarUsuario(Long id, Usuario master, String ip, String ua) {
        ...
        // Esta verificação se aplica a qualquer usuário que precisa de aprovação
        if (usuario.aprovado) {
